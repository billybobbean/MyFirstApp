...
class SliderView: UIView {
    ...
    weak var delegate: SliderViewProtocol? // делегат через которого мы будем работать дальше
    ...
    private var sliderType: SliderType? // свойство чтобы понимать, что это за вью
    
    public var isActive: Bool = true { // делаем неактивный элемент полупрозрачным и ставим его значение на 0
        didSet {
            if self.isActive {
                nameLabel.alpha = 1
                numberLabel.alpha = 1
